...
void loop() {
  //Sensamos el valor de corriente del primer sensor.
  //Obtenemos el valor de la corriente de cada sensor.
  int mA1 = ACS1.mA_AC();

  //-------------------------------FUNCION ENCARGADA DE CALCULCAR VALORES DE CORREINTE-------------------------------.
  calcular_corriente();
...
//FUNCIONES PARA LA SINCRONIZACION
// Función que retorna el valor actual de millis()
unsigned long obtenerTiempoActual() {
...
//Esta funcion se llama cada vez que se detecto un cambio de estado en los colores.
void reenvia_paquete(){
  //recibir el valor recibido del nodo anterior

  if(//si recibo valores en nodo anterior){
    //valido que valor tengo actualmente en True de mis baceras de los focos.

    if(rojo){
      //armo el mensaje con la info anterior y el valor del foco prendido en el momento. Esto por cada color
    }
  }
}
